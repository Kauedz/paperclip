...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("althius-worktree-enabled") !== "true") return null;

  const name = readMetaContent("althius-worktree-name");
  const color = normalizeHexColor(readMetaContent("althius-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("althius-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
